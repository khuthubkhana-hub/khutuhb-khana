...
const ReportsModal: React.FC<ReportsModalProps> = ({ onClose }) => {
    ...
    try {
      if (activeTab === 'Most Read') {
        ...
        data?.forEach((item: any) => {
          if (item.book_id && item.books) {
            counts[item.book_id] = (counts[item.book_id] || 0) + 1;
            bookDetails[item.book_id] = item.books;
          }
        });
        ...
        const { data, error } = await supabase
          .from('circulation')
          .select('member_id, members(name, class, register_number)');
        ...
        data?.forEach((item: any) => {
          if (item.member_id && item.members) {
            counts[item.member_id] = (counts[item.member_id] || 0) + 1;
            memberDetails[item.member_id] = item.members;
          }
        });
        ...
        const sortedMembers = Object.entries(counts)
          .map(([memberId, count]) => ({
            ...
            class: memberDetails[memberId].class,
            register_number: memberDetails[memberId].register_number
          }))
  ...
  const filteredData = useMemo(() => {
    const data = activeTab === 'Most Read' ? mostRead : mostActive;
    ...
    if (activeTab === 'Most Read') {
      ...
      return data.filter(item =>
        ...
        item.register_number?.toLowerCase().includes(lowerSearch)
      );
    }
  }, [activeTab, mostRead, mostActive, searchTerm]);

  const filteredDetailData = useMemo(() => {
    if (!searchTerm || !detailData) return detailData;
    const lowerSearch = searchTerm.toLowerCase();
    ...
    if (detailView?.type === 'book') {
      return detailData.filter(item =>
        ...
        item.members?.register_number?.toLowerCase().includes(lowerSearch)
      );
    } else {
      return detailData.filter(item =>
        item.books?.title?.toLowerCase().includes(lowerSearch) ||
        item.books?.author?.toLowerCase().includes(lowerSearch)
      );
    }
  }, [detailData, searchTerm, detailView]);

  const renderListView = () => {
    let emptyIcon, emptyMessage;
    switch (activeTab) {
      case 'Most Read':
        emptyIcon = BarChart2; emptyMessage = "No borrowing data available for this period.";
        break;
      case 'Most Active':
        emptyIcon = Users; emptyMessage = "No active readers found for this period.";
        break;
    }
    ...
    if (error) return (
      <div className="text-center text-accent-light py-10 flex flex-col items-center justify-center h-64 animate-fade-in">
        ...
        <p className="font-bold text-white text-xl mb-2">Error Loading Data</p>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                  <div className={`flex items-center justify-center w-8 h-8 font-bold text-lg ${rankColor} flex-shrink-0`}>
                    {isTop3 ? <Trophy size={20} /> : index + 1}
                  </div>
                  <div className="p-3 bg-neutral-800 rounded-lg group-hover:bg-neutral-700 transition-colors flex-shrink-0">
                    <BookOpen className="text-primary" size={24} />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-neutral-100 text-base sm:text-lg group-hover:text-primary transition-colors truncate">{item.title}</p>
                    <p className="text-sm text-neutral-400 truncate">by {item.author}</p>
                  </div>
                </div>
                <div className="text-right pl-4 flex-shrink-0">
                  <div className="flex flex-col items-end">
                    <span className="font-bold text-xl sm:text-2xl text-white">{item.read_count}</span>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-neutral-100 text-base sm:text-lg group-hover:text-accent transition-colors truncate">{item.name}</p>
                    <p className="text-sm text-neutral-400 flex items-center gap-2 flex-wrap">
                      ...
                      <span className="truncate">#{item.register_number || 'N/A'}</span>
                    </p>
                  </div>
                </div>
                <div className="text-right pl-4 flex-shrink-0">
                  <div className="flex flex-col items-end">
                    <span className="font-bold text-xl sm:text-2xl text-white">{item.book_count}</span>
    ...
    return (
      <ul className="space-y-2">
        {filteredDetailData.map((item, index) => (
          <motion.li
            key={item.id || index}
            className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 bg-neutral-800/40 border border-neutral-800 rounded-lg hover:bg-neutral-800 transition-colors gap-3"
            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}
          >
            {detailView.type === 'book' ? (
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div className="w-10 h-10 bg-neutral-700 rounded-full flex items-center justify-center text-neutral-300 font-bold flex-shrink-0">
                  {item.members.name.charAt(0)}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-neutral-200 truncate">{item.members.name}</p>
                  <p className="text-sm text-neutral-500 truncate">{item.members.class || 'N/A'} • Reg: {item.members.register_number || 'N/A'}</p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div className="w-10 h-10 bg-neutral-700 rounded-lg flex items-center justify-center text-neutral-300 flex-shrink-0">
                  <BookOpen size={18} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-neutral-200 truncate">{item.books.title}</p>
                  <p className="text-sm text-neutral-500 truncate">by {item.books.author}</p>
                </div>
              </div>
            )}
            <div className="text-left sm:text-right text-sm text-neutral-400 flex items-center gap-2 flex-shrink-0">
              <Calendar size={14} />
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 z-50">
      <motion.div
        ...
        className="bg-neutral-900 border border-neutral-700 text-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[95vh] sm:max-h-[90vh] flex flex-col overflow-hidden"
      >
        <div className="flex justify-between items-center p-4 sm:p-6 border-b border-neutral-800 bg-neutral-900/50">
          <div className="flex items-center gap-3">
            <div className="bg-primary/20 p-2 rounded-lg">
              <TrendingUp className="text-primary" size={24} />
            </div>
            <div>
              <h2 className="text-lg sm:text-xl font-bold text-white">Library Insights</h2>
              <p className="text-xs text-neutral-400 hidden sm:block">Real-time analytics and reports</p>
            </div>
          </div>
        ...
        <AnimatePresence mode="wait">
          {!detailView ? (
            <motion.div key="list-view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="border-b border-neutral-800 bg-neutral-900/30 overflow-x-auto">
                <nav className="flex gap-4 sm:gap-6 px-4 sm:px-6 min-w-max">
                  {(['Most Read', 'Most Active'] as ReportTab[]).map(tab => (
                    <button
                      key={tab}
                      onClick={() => { setActiveTab(tab); setSearchTerm(''); }}
                      className={`py-4 px-2 font-medium text-sm transition-all relative whitespace-nowrap ${activeTab === tab ? 'text-primary' : 'text-neutral-400 hover:text-white'}`}
                    >
                      {tab}
                      {activeTab === tab && (
        ...
        <div className="p-4 sm:p-6 flex-grow overflow-y-auto custom-scrollbar">
          <AnimatePresence mode="wait">
            ...
            >
              {detailView ? (
                <div className="mb-6">
                  <h3 className="text-xl sm:text-2xl font-bold text-white mb-1 truncate">{detailView.type === 'book' ? detailView.item.title : detailView.item.name}</h3>
              ...
              <div className="flex flex-col gap-4 mb-6">
                ...
                {!detailView && (
                  <div className="flex items-center gap-1 bg-neutral-800 p-1 rounded-xl w-full overflow-x-auto">
                    {(['all', 'year', 'month', 'week'] as TimeFilter[]).map(filter => (
                      <button
                        key={filter}
                        onClick={() => setTimeFilter(filter)}
                        className={`flex-1 px-3 sm:px-4 py-1.5 text-xs font-bold uppercase tracking-wider rounded-lg transition-all whitespace-nowrap ${timeFilter === filter ? 'bg-neutral-700 text-white shadow-md' : 'text-neutral-500 hover:text-neutral-300 hover:bg-neutral-700/50'}`}
                      >
                        {filter}
                      </button>
                    ))}
