...
import { Printer, Book as BookIcon, Users, Folder, AlertCircle, User, Star } from 'lucide-react';
...
const LibraryStatistics: React.FC<LibraryStatisticsProps> = ({ books, members, circulation, categories }) => {
  const stats = useMemo(() => {
    ...
    const booksPerLanguage = books.reduce((acc, book) => {
      const lang = book.language || 'Unknown';
      acc[lang] = (acc[lang] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const languageStats = Object.entries(booksPerLanguage)
    ...
    <div>
      ...
      <div id="statistics-printable-area">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <StatCard icon={BookIcon} value={stats.totalBooks} label="Total Books" color="bg-primary" />
          ...
          <StatCard icon={BookIcon} value={stats.booksIssued} label="Books Currently Issued" color="bg-indigo-500" />
        ...
        <div className="mt-8">
          <h2 className="text-lg font-semibold mb-4">Books per Language</h2>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-neutral-200">
              <thead className="bg-neutral-50">
                <tr>
              ...
              <tbody className="bg-white divide-y divide-neutral-200">
                {stats.languageStats.map(lang => (
                  <tr key={lang.name}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-neutral-900">{lang.name}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-neutral-500">{lang.count}</td>
                  </tr>
                ))}
              </tbody>
            </table>
